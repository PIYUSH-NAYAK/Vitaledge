...
const MedicineDetails = () => {
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_APP_BACKEND_URL || 'http://localhost:7777'}/api/cart/add`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        // No toast notification for cart operations - silent success
        console.log('Added to cart successfully');
        // Refresh cart count in real-time
        refreshCart();
        // Show that item was added
        setItemAdded(true);
      } else {
      ...
      // Show error toast for network/system errors
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-center space-x-3">
                    <button
                      ...
                      disabled={quantity <= 1}
                      className="w-8 h-8 bg-gray-700 border border-gray-600 rounded-md flex items-center justify-center text-white hover:bg-gray-600 disabled:opacity-50"
                    >
                      -
                    </button>
              ...
              {!itemAdded ? (
                <Button
                  onClick={addToCart}
                  disabled={medicine.stock.quantity === 0 || addingToCart}
                ...
                >
                  {addingToCart ? 'Adding...' : (medicine.stock.quantity > 0 ? 'Add to Cart' : 'Out of Stock')}
                </Button>
              ) : (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Button
                      ...
                      className="py-3 bg-gray-600 hover:bg-gray-700 text-white rounded-lg font-medium transition-colors duration-300"
                    >
                      Add More
                    </Button>
                    <Button
                      onClick={() => navigate('/cart')}
                      className="py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors duration-300"
                    >
                      Go to Cart
                    </Button>
                  </div>
