...
export const checkSignInMethods = async (email) => {
  try {
    ...
    if (!user) {
      // Fallback to checking by email if no current user
      const methods = await fetchSignInMethodsForEmail(auth, email);
      return {
        success: true,
        methods: methods
    ...
    // Check the current user's linked providers
    const providerMethods = user.providerData.map(provider => provider.providerId);
    
    // Also check if the user has a password set (this doesn't show in providerData)
    const emailMethods = await fetchSignInMethodsForEmail(auth, email);
    const hasPassword = emailMethods.includes('password');
    
    // Combine provider methods with password check
    const allMethods = [...providerMethods];
    if (hasPassword) {
